                ...
                int32_t *pn_heart_rate, int8_t *pch_hr_valid)
{
    ...
    // Threshold
    n_th1 = 0; 
    for (k = 0; k < BUFFER_SIZE; k++) n_th1 += an_x[k];
    n_th1 /= BUFFER_SIZE;
    ...
    // Peak detection, increase min_distance to 20 samples (~400 ms at 50 Hz)
    maxim_find_peaks(an_ir_valley_locs, &n_npks, an_x, BUFFER_SIZE, n_th1, 20, 15); 
    ...
    if (n_npks >= 2){
        ...
        int32_t hr = (int32_t)((FreqS * 60) / n_peak_interval_sum);

        // Validate HR: 30–180 bpm
        if(hr >= 30 && hr <= 180) {
            *pn_heart_rate = hr;
            *pch_hr_valid = 1;
        } else {
            *pn_heart_rate = hr;
            *pch_hr_valid = 0; // mark as invalid
        }
    }
    else { 
        *pn_heart_rate = -999; 
